...
const Cart = () => {
  ...
  const openRazorpayCheckout = async () => {
    try {
      const response = await axios.get("http://localhost:4000/api/getkey");
      ...
      const razorpay = new window.Razorpay({
        key: key,
        amount: totalPrice * 100, // Pass the total price in paisa (multiply by 100 for rupees)
        currency: "INR",
        name: "PALATE PLESEARS", // Update this to your company name
    ...
    <>
      <div className={`fixed right-0 top-0 w-full lg:w-[20vw] h-full p-5 bg-white mb-3 ${activeCart ? "translate-x-0" : "translate-x-full"} transition-all duration-500 z-50`}>
        ...
        <div className="absolute bottom-0">
          ...
          <button onClick={openRazorpayCheckout} className="bg-green-500 font-bold px-3 text-white py-2 rounded-lg w-[90vw] lg:w-[18vw] mb-5">Checkout</button>
        </div>
      </div>
